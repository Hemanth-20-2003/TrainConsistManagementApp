...
This use case introduces Binary Search for faster lookups in sorted data. By repeatedly dividing the search range in half, the algorithm efficiently finds the target bogie ID with significantly fewer comparisons.

## UC20: Exception Handling During Search Operations
This use case introduces defensive programming by preventing search operations on an empty train formation. If the bogie collection is empty, the system throws an `IllegalStateException` to stop execution immediately and notify the user of the invalid operation.
